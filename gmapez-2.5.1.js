 ...
 * 
 * 2013-Jan 2.5.1 DelGrosso <mikedelgrosso@gmail.com> Added the ability to use a select#ezmap-select to invoke map bubbles. 
 */
...
(function(){
  ...
  function loadfunc() {
    ...
    //begin extension for ezmap-select
    var select = document.getElementById("ezmap-select");
    if (!select.onchange){
      select.onchange = selectChange;
    }

    function selectChange() {
      var marker = markerForUrl( this.value );
      if( marker ) {
        renderMarker(marker);
        return false;
      }
    ...
    //end extension for ezmap-select
